...
Дашборд - [89.111.170.136:3000](http://89.111.170.136:3000/public/dashboard/5e630810-e564-4ecb-a9a8-af7ca6aafcad)
...
1. Создать виртуальный сервер, оптимальный объем RAM - 2 Гб.
...
8. Поменять пароль пользователя postgres, дабы избежать ошибок в будещем
...
10. Создать SSH-туннель к серверу и подключиться
...
nano nginx.conf
```
Слушаем порт, на котором запущена BI-система.
![](img/7.png)
...
18. Запустим скрипт, который первично заполнит базу всеми историческими данными, которые уже доступны в API на момент скрипта.

![](img/10.png)

19. Автоматизируем сбор данных за предыдущий день ежедневно в 7 утра
...
Сохраним и выйдем. Теперь скрипт будет запускаться ежедневно в 7:00.

20. Можно строить дашборд